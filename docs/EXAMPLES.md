...
- [Reference Semantics — `==`, `.equals()`, `copy()`, `id()`](#reference-semantics)
- [Number Conversions — hex, oct, bin, base64](#number-conversions)
- [CSV — parse and stringify](#csv)
...
> Arrays, dicts, and structs have **reference semantics** — `==` tests identity (same object), not contents. Use `.equals()` for structural comparison and `copy()` for a shallow clone. See [Reference Semantics](#reference-semantics) below.

---

## Reference Semantics — `==`, `.equals()`, `copy()`, `id()` {#reference-semantics}

Arrays, dicts, and structs are heap objects. Assignment copies the reference, not the value.
...
fn main() {
    ...
    let a = [1, 2, 3]
    let b = a           // same object
    let c = [1, 2, 3]   // different object, same contents
    println("a == b:", a == b)          // true  — same ref
    println("a == c:", a == c)          // false — different ref

    // .equals() is depth-1 structural
    ...
    println("p1 == p2:", p1 == p2)          // true  — same ref
    println("p1 == p3:", p1 == p3)          // false — different ref
    println("p1.equals(p3):", p1.equals(p3))    // true

    // copy() — depth-1 shallow clone
    let orig = [1, 2, 3]
    let dup = copy(orig)
    dup.push(99)
    println("orig after dup.push:", orig)   // [1, 2, 3] — outer independent

    let nested = [[1, 2], [3, 4]]
    let dup2 = copy(nested)
    dup2[0].push(99)
    println("nested[0] after dup2[0].push:", nested[0])  // [1, 2, 99] — inner shared
    ...
    println("id(x) == id(z):", id(x) == id(z))  // false — copy is new object
}
```
...
fn main() {
    ...
    println("hex(255):", hex(255))      // 0xff
    println("oct(64):", oct(64))        // 0o100
    println("bin(42):", bin(42))        // 0b101010
    println("hex(-1):", hex(-1))        // -0x1

    // Parse back with int(s, base)
    println("int(\"ff\", 16):", int("ff", 16))      // 255
    println("int(\"0xff\", 16):", int("0xff", 16))  // 255
    println("int(\"101\", 2):", int("101", 2))      // 5
    println("int(\"0b101\", 2):", int("0b101", 2))  // 5
    println("int(\"10\", 8):", int("10", 8))        // 8

    // Roundtrip
    let n = 255
    println("hex roundtrip:", int(hex(n), 16) == n)  // true
    println("bin roundtrip:", int(bin(n), 2) == n)   // true

    // base64
    let msg = "Hello, Aether!"
    let encoded = base64_encode(msg)
    let decoded = base64_decode(encoded)
    println("encoded:", encoded)
    println("decoded:", decoded)
    println("roundtrip:", decoded == msg)    // true
}
```
...
```aether
fn main() {
    // csv_parse — returns array of rows (each row is an array of strings)
    let text = "name,age,city\nAlice,30,New York\nBob,25,London"
    let rows = csv_parse(text)
    println("headers:", rows[0])        // [name, age, city]
    println("Alice's city:", rows[1][2])    // New York

    // Quoted fields with commas inside
    let messy = "\"Smith, John\",42,\"New York, NY\""
    let parsed = csv_parse(messy)
    println("quoted name:", parsed[0][0])   // Smith, John

    // Custom delimiter (TSV)
    let tsv = "a\tb\tc\n1\t2\t3"
    let tsv_rows = csv_parse(tsv, "\t")
    println("tsv row 1:", tsv_rows[1])      // [1, 2, 3]

    // csv_stringify — rows to CSV string
    let data = [
        ["product", "price", "note"],
        ["Apple", "1.50", "fresh"],
    ...
    // Roundtrip
    let original = [["city", "note"], ["New York", "big, busy"]]
    let encoded = csv_stringify(original)
    let decoded = csv_parse(encoded)
    println("roundtrip:", decoded.equals(original))  // true
}
```
